...
/**
 * Observer for salesrule save after event
 */
class AdminhtmlSalesruleSaveAfter implements ObserverInterface
{
    /**
    ...
    public function execute(Observer $observer)
    {
        try {
            ...
            if ($rule && $rule->getRuleId()) {
                $this->logger->info(
                    sprintf('SalesRule saved: %d', $rule->getRuleId())
                );

                // Publish message to queue for async processing
                $this->promoHintPublisher->publish($rule->getRuleId(), 'save');
            }
        } catch (\Exception $e) {
            $this->logger->error(
                sprintf('Error in salesrule save observer: %s', $e->getMessage()),
                ['exception' => $e]
            );
        }
    }
